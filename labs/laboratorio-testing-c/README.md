...
> R:Se encarga de que la cantidad de producto empiece en 0, es importante ya que tiene que limpiar el carrito de productos anteriores
...
> R:para calcular correctamente se tendria que multiplicar por la cantidad
...
> R:falla. Devuelve un 1 y despues tira un error
...
> R:el operador incorrecto era el <=, deberia ir <
...
> R: si la 29 y la 30. No se ejecuta ya que no escribimos ninugn test en test_unitario.c que llame a esta funcion
...
> R:el test unitario prueba una sola funcion, y el test de integracion prueba varias funciones. lo detecto primero el test unitario
...
> R:es peligroso en c ya que te deja escribir directamente en la memoria, java por ejemplo tiene verificacion de limites automaticas.
...
> R:yo diria que es mas rapido, en vez de leer linea por linea, el test te demuestra si funciona o no.
...
> R: porque al ser cantidad 1, el precio base era igual al precio multiplicado por la cantidad.
